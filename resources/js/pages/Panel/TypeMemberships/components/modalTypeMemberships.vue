<template>
    <Dialog :open="statusModal" @update:open="closeModal">
        <DialogContent>
            <DialogHeader>
                <DialogTitle>
                    {{ isEditing ? 'Editar tipo de membresia' : 'Nuevo tipo de membresia' }}
                </DialogTitle>
                <DialogDescription>Asegúrate de que la información sea correcta</DialogDescription>
            </DialogHeader>

            <!-- Formulario -->
            <form @submit.prevent="submitForm" class="flex flex-col gap-4 py-3">

                <!-- Ingresar el nombre -->
                <FormField v-slot="{ componentField }" name="name">
                    <FormItem>
                        <FormLabel>Nombre de la membresia</FormLabel>
                        <FormControl>
                            <Input id="name" type="text" v-bind="componentField"/>
                        </FormControl>
                        <FormMessage/>
                    </FormItem>
                </FormField>

                <!-- Ingresar la descripción -->
                <FormField v-slot="{ componentField }" name="description">
                    <FormItem>
                        <FormLabel>Descripción</FormLabel>
                        <FormControl>
                            <Input id="description" type="text" v-bind="componentField"/>
                        </FormControl>
                        <FormMessage/>
                    </FormItem>
                </FormField>

                <!-- Ingresar el precio -->
                <FormField v-slot="{ componentField }" name="price">
                    <FormItem>
                        <FormLabel>Precio</FormLabel>
                        <FormControl>
                            <Input id="price" type="number" step="0.01" v-bind="componentField"/>
                        </FormControl>
                        <FormMessage/>
                    </FormItem>
                </FormField>

                <!-- Ingresar el estado -->
                <FormField v-slot="{ value, handleChange }" type="checkbox" name="status">
                    <FormItem class="flex flex-row items-start space-y-0 gap-x-3 rounded-md border p-4">
                        <FormControl> <Checkbox :model-value="value" @update:model-value="handleChange" /> </FormControl>
                        <div class="space-y-1 leading-none">
                            <FormLabel>Estado</FormLabel>
                            <FormMessage />
                        </div>
                    </FormItem>
                </FormField>

                <!-- Botones -->
                <DialogFooter>
                    <Button type="submit" variant="default"> Guardar </Button>
                    <Button type="button" variant="secondary" @click="closeModal">Cancelar</Button>
                </DialogFooter>

            </form>
        </DialogContent>
    </Dialog>
</template>

<script setup lang="ts">
import Dialog from '@/components/ui/dialog/Dialog.vue';
import { storeTypeMembershipRequest, TypeMembershipFormData, TypeMembershipResource, updateTypeMembershipRequest } from '../interfaces/TypeMembership';
import { computed, watch } from 'vue';
import { z } from 'zod';
import { toTypedSchema } from '@vee-validate/zod';
import { useForm } from 'vee-validate';
import DialogHeader from '@/components/ui/dialog/DialogHeader.vue';
import DialogTitle from '@/components/ui/dialog/DialogTitle.vue';
import DialogContent from '@/components/ui/dialog/DialogContent.vue';
import DialogDescription from '@/components/ui/dialog/DialogDescription.vue';
import { FormField } from '@/components/ui/form';
import FormItem from '@/components/ui/form/FormItem.vue';
import FormLabel from '@/components/ui/form/FormLabel.vue';
import FormControl from '@/components/ui/form/FormControl.vue';
import Input from '@/components/ui/input/Input.vue';
import FormMessage from '@/components/ui/form/FormMessage.vue';
import DialogFooter from '@/components/ui/dialog/DialogFooter.vue';
import Button from '@/components/ui/button/Button.vue';
import Checkbox from '@/components/ui/checkbox/Checkbox.vue';

interface Props{
    statusModal: boolean;
    typeMembership?: TypeMembershipResource | null;
}

const props = defineProps<Props>();

const emit = defineEmits<{
    (e: 'close-modal', open: boolean): void;
    (e: 'create', data: storeTypeMembershipRequest): void;
    (e: 'update', data: updateTypeMembershipRequest): void;
}>();

// Computed para determinar si estamos editando
const isEditing = computed(() => !!props.typeMembership?.id);

const closeModal = () => {
    emit('close-modal', false);
};

const formShema = toTypedSchema(
    z.object({
        name: z.string().min(3, 'El nombre de la membresia es requerido'),
        description: z.string().min(3, 'La descripción es requerida'),
        price: z.number().min(0, 'El precio debe ser mayor o igual a 10'),
        status: z.boolean(),
    })
);

const { handleSubmit, setValues } = useForm({
    validationSchema: formShema,
    initialValues: {
        name: props.typeMembership?.name || '',
        description: props.typeMembership?.description || '',
        price: props.typeMembership?.price || 0,
        status: props.typeMembership?.status || false,
    },
})

const submitForm = handleSubmit((values: TypeMembershipFormData) => {
    console.log('Formulario enviado: ', values);
    if(props.typeMembership) {
        const updateTypeMembership: updateTypeMembershipRequest = {
            ...values,
            id: props.typeMembership.id,
        };
        emit('update', updateTypeMembership);
    }else {
        const newTypeMembership: storeTypeMembershipRequest = {
            ...values,
        };
        emit('create', newTypeMembership);
    }
});

watch(
  () => props.typeMembership,
  (newValue) => {
    if (newValue) {
      setValues({
        name: newValue.name ?? '',
        description: newValue.description ?? '',
        price: newValue.price ?? 0,
        status: newValue.status ?? false,
      });
    }
  },
);
</script>

<style scoped> 
</style>