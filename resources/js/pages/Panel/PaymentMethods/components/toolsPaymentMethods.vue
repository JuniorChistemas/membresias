<template>
    <div class="flex flex-wrap items-center gap-3 p-2">
        <!-- Exportar Excel -->
        <a href="/panel/reports/export-excel-paymentMethods" download>
            <Button
                variant="ghost"
                size="sm"
        ...
        <!-- Exportar PDF -->
        <a href="/panel/reports/export-pdf-paymentMethods" download>
            <Button
                variant="destructive"
...
const handleFileChange = async (event: Event) => {
    ...
    try {
        await axios.post('/panel/reports/import-excel-paymentMethods', formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        })
        toast.success('¡Importación exitosa! Los metodos de pago fueron importados correctamente.')
        emit('import-success')
        target.value = ''
    } catch (error) {
